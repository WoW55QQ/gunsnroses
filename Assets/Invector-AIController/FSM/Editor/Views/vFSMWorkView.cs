    ...
    public class vFSMWorkView : FSMViewBase
    {
        ...
        void ProcessContexMenu(Event e, int contexID)
        {
            ...
            if (contexID == 0)
            {
                if (currentFSM != null)
                {
                    var possibleStates = FindSubClassesOfNode<vFSMState>().ToList();
                    menu.AddItem(new GUIContent($"New State"), false, () => { vNodeUtility.CreateNode(currentFSM, mousePosition); });
                    for (int i=0;i<possibleStates.Count;i++)
                    {
                        Type type = possibleStates[i];
                        string name = possibleStates[i].Name;                      
                        menu.AddItem(new GUIContent($"New {name}"), false, () => { vNodeUtility.CreateNode(type,currentFSM, mousePosition); });
                    }
            ...
            if (contexID == 1)
            {
                if (currentFSM != null)
                {
                    ...
                    if ((selectedStates.Count<=1 && currentFSM.states[selectedNodeID].canRemove) || (selectedStates.Count>1 && selectedStates.Find(s=>s.canRemove)))
                        menu.AddItem(new GUIContent("Delete Node"), false, () => { if (currentFSM.states[selectedNodeID].canRemove) vNodeUtility.DeleteNode(selectedNodeID, currentFSM); foreach (var state in selectedStates)if(state.canRemove) vNodeUtility.DeleteNode(currentFSM.states.IndexOf(state), currentFSM); });
